...
In order to make authorized calls to Todoist APIs, your application must first obtain an access token from the users. This section describes different ways of obtaining such a token.

Note that we encourage your application to use OAuth protocol to obtain access token from the user, as the other authentication methods (`login` and `login_with_google`) are scheduled for deprecation.
...
You can also obtain your current API token through the Todoist web application at:
